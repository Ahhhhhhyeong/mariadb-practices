...
SELECT 
    b.emp_no, a.first_name, MAX(b.salary) AS salary
FROM
    salaries b
        INNER JOIN
    employees a ON b.emp_no = a.emp_no
GROUP BY b.emp_no , a.first_name
...
SELECT 
    a.emp_no, a.first_name, max(c.dept_name) as dept_name
FROM
    employees a
        LEFT JOIN
    dept_emp b ON a.emp_no = b.emp_no
...
group by a.emp_no, a.first_name;
...
FROM
    employees e
        JOIN
    titles a ON e.emp_no = a.emp_no
WHERE
    a.title = 'Technique Leader'
         AND a.to_date not in ('9999-01-01');
...
SELECT 
    a.emp_no, c.first_name, b.salary
FROM
    titles a,
    salaries b,
    employees c
...
SELECT 
    a.title, avg(b.salary) as salary
FROM
    titles a,
    salaries b
WHERE
